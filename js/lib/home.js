mixins.home = {
    mounted() {
        ...
        homeBackgroundRef.style.backgroundImage = `url('${homeBgImgURLs[Math.floor(Math.random() * homeBgImgURLs.length)]
            }')`;
        this.menuColor = true;

        let contentBackgroundRef = this.$refs.contentBackground;
        let contentBgImgURLs = contentBackgroundRef.dataset.images.split(",");
        contentBackgroundRef.style.backgroundImage = `url('${contentBgImgURLs[Math.floor(Math.random() * contentBgImgURLs.length)]
            }')`;
    },
    methods: {
        homeClick() {
